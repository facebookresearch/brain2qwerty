...
Reads a predictions CSV (as produced by extract_predictions.py) containing
per-sentence logits, applies character-level beam search with a KenLM
N-gram model, and outputs an updated CSV with refined predictions and
recomputed CER/WER.

Input:
    A CSV with columns: Subject, True Sentences, Logits
    (e.g., the output of extract_predictions.py)

Output:
    A CSV with additional columns: LM Predictions, CER_LM, WER_LM

Usage:
    python ngram_decoding.py --input predictions.csv --lm news_9gram.arpa --output predictions_lm.csv
    python ngram_decoding.py --input predictions.csv --lm news_9gram.arpa  # overwrites input
"""
...
class BeamDecoder:
    """Character-level beam search decoder with KenLM language model."""
    ...
    def decode(self, emissions: torch.Tensor) -> str:
        ...
        for logits in emissions:
            ...
            top_indices = probs.argsort(descending=True)[: self.max_labels_per_timestep]
            ...
            for hyp in beam:
                for idx in top_indices:
                    ...
                    new_score = hyp.score + self.lm_weight * lm_score + brain_score
...
def main():
    parser = argparse.ArgumentParser(
        description="N-gram LM post-processing for Brain2Qwerty predictions"
    )
    parser.add_argument("--input", required=True, help="Input predictions CSV")
    parser.add_argument("--lm", required=True, help="Path to KenLM .arpa language model")
    parser.add_argument(
        "--output", default=None, help="Output CSV path (default: overwrite input)"
    )
    parser.add_argument(
        "--lm-weight",
        type=float,
    ...
    parser.add_argument(
        "--beam-size", type=int, default=30, help="Beam size (default: 30)"
    )
    ...
    print(f"  {len(df)} sentences, {df['Subject'].nunique()} subjects")
    ...
    df["WER_LM"] = df.apply(
        lambda r: (
            Levenshtein.distance(r["True Sentences"].split(), r["LM Predictions"].split())
            / max(len(r["True Sentences"].split()), 1)
        ),
        axis=1,
    )

    print(f"\n{'Subject':<12} {'CER (model)':>12} {'CER (LM)':>10} {'WER (LM)':>10}")
    print("-" * 46)
    for subject in sorted(df["Subject"].unique()):
        sdf = df[df["Subject"] == subject]
        print(
            f"{subject:<12} {sdf['CER'].mean():>12.3f} {sdf['CER_LM'].mean():>10.3f} {sdf['WER_LM'].mean():>10.3f}"
        )

    per_subj = df.groupby("Subject")[["CER", "CER_LM", "WER_LM"]].mean()
    print("-" * 46)
    print(
        f"{'Overall':<12} {per_subj['CER'].mean():>12.3f} {per_subj['CER_LM'].mean():>10.3f} {per_subj['WER_LM'].mean():>10.3f}"
    )
