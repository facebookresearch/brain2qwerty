...
Official implementation of **Accurate Decoding of Natural Sentences from Non-Invasive Brain Recordings** (paper under review; link to follow).
...
- The released configuration that reproduces the paper
- The full evaluation pipeline to retrieve ordered, per-subject results
...
Per-epoch metrics are written to `logs/` (CSV) and the progress bar reports every loss and metric, so runs stay legible without Weights & Biases. At test time the `PredictionCSVCallback` saves per-sentence predictions to `predictions_test.csv`, scored with three metrics: character error rate (CER), word error rate (WER) and semantic error rate (SemER, a RoBERTa-large embedding distance).
