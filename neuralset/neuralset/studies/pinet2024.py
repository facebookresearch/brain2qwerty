...
class _Pinet2024(BaseData):
    ...
    description: tp.ClassVar[str] = (
        "MEG/EEG recordings collected during a copy-typing task. "
        "Participants view phrases on screen, then type them on a keyboard "
        "without character-level visual feedback. "
        "Use Pinet2024Meg for MEG data, Pinet2024Eeg for EEG data."
    )
    ...
    def _read_log(self, log_fname: Path) -> pd.DataFrame:
        ...
        for event_type in ("key", "rsvp"):
            for trial_id, trial in enumerate(struct[event_type]):
                ...
                trial["true_sequence"] = _clean_seq(struct["sequence"][trial_id])
    ...
    def _load_events(self):
        ...
        meta1 = meta[meta.is_image == True]
        meta2 = meta[meta.is_image == False]
...
class Pinet2024Meg(_Pinet2024):
    ...
    @lru_cache
    def _get_all_files(cls, path) -> pd.DataFrame:
        """Convenience class to prepare a DataFrame of valid Meg and log filenames for Pinet2024."""
...
class Pinet2024Eeg(_Pinet2024):
    ...
    @lru_cache
    def _get_all_files(cls, path) -> pd.DataFrame:
        """Convenience class to prepare a DataFrame of valid EEG and log filenames for Pinet2024."""
        BADS = [  # Known bad files
